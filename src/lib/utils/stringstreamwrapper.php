...
class StringStreamWrapper {
    /**
     * Wrap a string into a "php://memory" stream
     *
    ...
    public static function Open($string) {
        ...
        if (($stream = fopen('php://memory', 'r+')) === false)
            throw new FatalException('StringStreamWrapper: fopen "php://memory" failed');
        if (fwrite($stream, $string) === false)
            throw new FatalException('StringStreamWrapper: fwrite failed');
        if (rewind($stream) === false)
            throw new FatalException('StringStreamWrapper: rewind failed');
        return $stream;
    }
}
